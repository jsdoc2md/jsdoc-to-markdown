...
class Jsdoc2md extends EventEmitter {
  ...
  buildTemplate (array, options) {
    ...
    const decs = [
      'dmd2/lib/decoration/index',
      'dmd2/lib/decoration/body',
      'dmd2/lib/decoration/signature',
      'dmd2/lib/decoration/signature-function',
      'dmd2/lib/decoration/signature-class',
      'dmd2/lib/decoration/signature-constructor',
      'dmd2/lib/decoration/signature-module'
    ].forEach(dec => {
      this.emit('progress', 'Loading built-in ' + dec)
      options.decorations.push(require(dec))
    })
    ...
    if (options.format) {
      ...
      if (options.term) {
        jsdoc2md.emit('progress', 'Loading dterm')
        const dterm = require('dterm')
        Template = dterm
      } else {
        jsdoc2md.emit('progress', 'Loading dmd')
...
class JsdocOptions {
  constructor (options) {
    ...
    this.decorations = loadDecorations(this.decorations)
  }
}

function loadDecorations (decorationOptions) {
  jsdoc2md.emit('progress', 'Loading decorations')
  const walkBack = require('walk-back')
  const fs = require('fs')

  /* each decoration can be an absolute path or a module name */
  return arrayify(decorationOptions).map(function (decorationPath) {
    if (t.isFunction(decorationPath)) {
      return decorationPath
    } else if (fs.existsSync(path.resolve(decorationPath))) {
      ...
      if (decorationModulePath) {
        ...
        throw new Error(`Invalid decoration module: ${decorationPath}`)
      }
    }
  })
}
