...
const CACHE_DIR = path.resolve(homePath(), '.jsdoc2md')
/**
...
function getJsdoc (src, options) {
  ...
  const promises = arrayify(src).map(file => fs.readFile(file))

  return Promise.all(promises)
    .then(contents => {
      contents.forEach(content => hash.update(content))
      const checksum = hash.digest('hex')
      const cachePath = path.resolve(CACHE_DIR, checksum)
      ...
      try {
        ...
        return jsdocApi.explain(jsdocOptions)
          .then(explainOutput => {
            fs.writeFileSync(cachePath, JSON.stringify(explainOutput, null, '  '))
